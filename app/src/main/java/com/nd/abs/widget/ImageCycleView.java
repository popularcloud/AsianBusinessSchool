...
//				}
	...
	private final class GuidePageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int index) {
			...
			if (index == 0 || index == mAdvAdapter.getCount() + 1) {
				return;
			}
			// 设置图片滚动指示器背景
			mImageIndex = index;
			...
			mImageViews[index].setBackgroundResource(R.mipmap.icon_point);
	...
	private class ImageCycleAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			...
			String imageUrl = mAdList.get(position).getAdvertisingImageUrl();
			...
			imageView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					mImageCycleViewListener.onImageClick(mAdList.get(i),i, v);
				}
			});
